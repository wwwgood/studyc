...
|------|------|
| 🏠 总览 | 三科进度汇总、今日打卡状态 |
| 💻 信息学 | 26 关闯关地图、徽章墙、训练基地 7 面板、赛事航路、家长指挥所、**实战特训**（11 道机考真题 + 半年航线 + 作战手册）、**学校练习**（《一本通》同步：做题→解答→解析 + 徽章墙考核 + 投稿箱） |
...
- 解析含「逐行讲解」walk 块：一行代码配一句解析，代码与解析不再分离
- 三个视图：「📝 练习题目」/「🏆 练习徽章」（19 枚题目徽章 + 分组进度 + 抽一题考我，照徽章墙做法考核）/「📥 投稿箱」（格式不限粘贴新内容，攒多了一键复制发给 AI 整理成下一课）
- 数据同步弹窗升级：GitHub 云同步置顶为推荐通道 + 「⚡ 一键打开 GitHub 令牌创建页」直达链接（已预勾 gist 权限），配置一次后平板/电脑自动上云、一键取回
...
| 🧩 选词填空 | `词库：tall taller tallest`（或 `题干词汇：…`，词项用全角空格分隔）+ 带空小题 + `答案：1. taller 2. tallest`；无空位时按答案自动生成空槽 | 点击词库单词填入空格 → 提交逐空判题 |
