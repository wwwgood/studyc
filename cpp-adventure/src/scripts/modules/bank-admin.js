...
var BA_SECTION_TYPES = [
  ...
  { keys: ["改错", "纠错", "改正错误", "找出.*错误"], type: "fill" },
...
var BA_KNOWLEDGE_MAP = [
  ...
  { module:"grammar", topicId:1, name:"专有名词", how:/专有名词|人名|地名|国名|国家名称|月份名称|节日名称|江河名称|姓在前|拼音要连写/ },
  { module:"grammar", topicId:1, name:"名词作定语", how:/作定语|名词作定语|修饰另一个名词|修饰.*名词/ },
  { module:"grammar", topicId:1, name:"名词所有格", how:/所有格|所有关系|共同拥有|of\s*结构|在.*后加\s*['’]?s/ },
  { module:"grammar", topicId:1, name:"名词复数-es规则", how:/加\s*es|以.*(s|x|ch|sh).*结尾/ },
  { module:"grammar", topicId:1, name:"名词复数-ies规则", how:/y.*变.*ies|辅音字母.*y/ },
  { module:"grammar", topicId:1, name:"名词复数-ves规则", how:/f.*变.*ves|f\/fe\s*结尾|变\s*f\/fe/ },
  { module:"grammar", topicId:1, name:"不规则名词复数", how:/不规则变化|不规则复数|单复数同形|特殊.*复数|复合名词|中心词.*变/, howQ:/\b(men|women|children|feet|teeth|mice|sheep|deer|people)\b/ },
  { module:"grammar", topicId:1, name:"可数与不可数名词", how:/不可数名词|uncountable|集合名词/, howQ:/How much.*(water|bread|milk|rice|tea|juice|money|paper|time|homework|news)/i, opts:/^(water|bread|milk|rice|tea|juice|money|paper|snow|hair|homework|news|advice)$/i },
  { module:"grammar", topicId:1, name:"名词复数-加s规则", how:/直接加\s*-?s|一般.*加\s*s|变复数|用复数|表示一类|不用复数|元音字母.*y/, howQ:/\b(two|three|four|five|many|some|lots of)\b.*\b(bananas|birds|books|dogs|pens|cats|apples|bags|eggs)\b/i },
  { module:"grammar", topicId:1, name:"名词固定搭配", how:/固定搭配|on foot|a pair of|the key to/ },
  { module:"grammar", topicId:1, name:"名词辨认", how:/名词辨析|考查名词|词性|名词辨认|哪个.*是名词/, opts:/^(run|jump|eat|happy|sad|big)$/ },
  /* ---- 第2章 冠词湾 ---- */
  ...
  /* ---- 第7章 介词迷宫 ---- */
  { module:"grammar", topicId:7, name:"时间介词", how:/时间介词|介词.*时间/, howQ:/(___+|＿＿+|（\s*）|\(\s*\))\s*(in|on|at)\b|(in|on|at)\b\s*(___+|＿＿+|（\s*）|\(\s*\))|(___+|＿＿+)\s*(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|morning|afternoon|evening|night)/i, opts:/^(in|on|at)$/i },
  ...
  { module:"grammar", topicId:9, name:"there be句型", how:/there\s*be|There\s*be/i, howQ:/\bthere\s+(___+|＿＿+)/i, opts:/^(is|are|was|were|has|have)$/i },
...
var BA_WHY_TPL = {
  ...
  "专有名词": "本题考查专有名词：专有名词是特定的人、地名、国家、月份、节日等的名称，首字母要大写。人名如 Sally Green、地名如 Hong Kong、江河如 the Yellow River、月份如 May、节日如 National Day。{answer}",
  "名词作定语": "本题考查名词作定语：名词修饰另一个名词时通常用单数形式，如 apple trees（苹果树）、apple pies（苹果派）。{answer}",
  "名词固定搭配": "本题考查名词固定搭配：如 on foot（步行）、a pair of glasses（一副眼镜）、the key to the door（房门的钥匙），这些搭配要整体记忆。{answer}",
...
function baSmartParse(text){
  ...
  bigBlocks.forEach(function(block){
    ...
    /* 块内含词库/题干词汇行 → 整块按选词填空解析 */
    if (/(?:^|\n)\s*(题干词汇|词库|方框|用方框|选词)[：:]/.test(block.text)) sectionType = "cloze";
    ...
    var whole = (sectionType === "cloze" || sectionType === "reading" || sectionType === "essay" ||
                ...
                /(?:^|\n)\s*(题干词汇|词库|方框|用方框|选词)[：:]/.test(block.text);
    if (whole){
...
function baParseFill(lines, fullText){
  ...
  if (ansLine){
    ...
    if (am){
      ansText = am.split(/解析[：:]/)[0].trim();
      if (ansText.indexOf("→") >= 0) ansText = ansText.split("→")[ansText.split("→").length - 1].trim();
      ansSource = "原卷行内";
    }
  }
  var whyLine = lines.find(function(l){ return /解析|解释|分析/i.test(l); });
...
function baParseCloze(lines, fullText){
  ...
  var answersByPos = {};
  lines.forEach(function(line){
    var m = line.match(/^(?:题干词汇|词库|方框|选词|单词)[：:]\s*(.+)$/);
    if (m){
      /* 词项拆分：含全角空格按全角拆（保留 Sally Green 等整词）；否则按半角/逗号拆 */
      if (m[1].indexOf("　") >= 0){
        words = m[1].split(/　+/).map(function(w){ return w.trim(); }).filter(function(w){ return w && /^[A-Za-z][A-Za-z\-']*/.test(w); });
      } else {
        words = m[1].split(/[,，、\s]+/).map(function(w){ return w.trim(); }).filter(function(w){ return w && /^[A-Za-z][A-Za-z\-']*/.test(w); });
      }
  ...
  if (qTexts.length === 0){
    ...
    if (blankCount === 0){
      /* 无空位：按「答案：xx（释义）」逐行提取答案作空槽 */
      lines.forEach(function(line){
        var am2 = line.match(/答案[：:]\s*([^（(]+)/);
        if (am2){
          var v = am2[1].split(/解析[：:]/)[0].trim();
          if (v && blanks.indexOf(v) < 0) blanks.push(v);
        }
      });
    } else {
